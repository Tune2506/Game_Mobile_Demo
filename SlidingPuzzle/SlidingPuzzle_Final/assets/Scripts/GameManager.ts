import { _decorator, Component, Node, Label, Vec3, UITransform, Size } from 'cc';
...
export class GameManager extends Component {
    ...
    public startGame() {
        ...
        for (let i = 0; i < this.tileNodes.length; i++) {
            ...
            // Cập nhật chữ số trên Label con của tile
    ...
    /** Đặt lưới về trạng thái giải xong chuẩn: 1…(N²-1), ô trống ở góc dưới phải */
    private _resetGrid() {
        const n = this.gridSize;
        this._grid = [];
        let num = 1;
        for (let r = 0; r < n; r++) {
            this._grid[r] = [];
            for (let c = 0; c < n; c++) {
                // Ô cuối cùng (góc dưới phải) là ô trống (0)
                this._grid[r][c] = (r === n - 1 && c === n - 1) ? 0 : num++;
            }
        }
        this._emptyRow = n - 1;
        this._emptyCol = n - 1;
    ...
    /** Kiểm tra lưới hiện tại có phải trạng thái đã giải không */
    private _isSolved(): boolean {
        const n = this.gridSize;
        let num = 1;
        for (let r = 0; r < n; r++) {
            for (let c = 0; c < n; c++) {
                const expected = (r === n - 1 && c === n - 1) ? 0 : num++;
    ...
    private _updateMoveLabel() {
        if (this.moveCountLabel) {
            this.moveCountLabel.string = `Bước: ${this._moveCount}`;
        }
    }
}
